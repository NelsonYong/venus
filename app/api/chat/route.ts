import { convertToModelMessages, stepCountIs, streamText, UIMessage, experimental_generateImage as generateImage } from 'ai';
import { NextResponse } from 'next/server';
import { prisma } from '@/lib/prisma';
import { createModelAdapter, createImageModelAdapter, getDefaultModelConfig, isImageModel } from '@/lib/model-adapter';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 如果有上传的附件，将它们添加到最后一条用户消息中
    if (uploadedAttachments && uploadedAttachments.length > 0) {
      console.log('📎 处理上传的附件:', uploadedAttachments);

      const lastMessage = processedMessages[processedMessages.length - 1];
      if (lastMessage && lastMessage.role === 'user') {
        // 为图片文件创建 image parts
        const imageParts = uploadedAttachments
        ...
        console.log('🖼️ 添加图片到消息:', imageParts);
